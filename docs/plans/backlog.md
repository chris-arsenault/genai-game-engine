...
| --- | --- | --- | --- | --- |
| AR-050 | P0 | In Progress | Visual asset sourcing pipeline now covers Act 2 Crossroads selection conduit/pad, checkpoint glow/plaza, safehouse arc, boundary walls, and column beam overlays with CC0/CC-BY references captured in manifests; Session 112 calibrated tint/alpha parameters (~0.13 columns/conduits, ~0.75 boundaries), Session 113 added a lighting preview harness plus queued the AR-001–AR-005 generation batch, and Session 114 boosted conduit/glow/column luminance so all previewed segments now meet their preset thresholds. | Share the refreshed `reports/art/act2-crossroads-lighting-preview.json` with RenderOps for approval, then process the queued AR-001–AR-005 outputs to lock creators/licensing in `assets/images/requests.json`. |
