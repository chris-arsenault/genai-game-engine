...
| --- | --- | --- | --- | --- |
| AR-050 | P0 | In Progress | Visual asset sourcing pipeline now covers Act 2 Crossroads selection conduit/pad, checkpoint glow/plaza, safehouse arc, boundary walls, and column beam overlays with CC0/CC-BY references captured in manifests; Session 112 calibrated tint/alpha parameters (columns/conduits ~0.13 avg alpha, boundary shields ~0.75) and synced scene overrides with the regenerated derivatives. | Run an in-engine lighting review with RenderOps using the calibrated overlays, keep `overlayAverageAlpha` metadata aligned via `scripts/art/analyzeCrossroadsOverlays.js`, and trigger AR-001–AR-005 asset generation using the packaged prompts. |
