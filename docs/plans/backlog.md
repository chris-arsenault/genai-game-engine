...
- **Status**: Memory Parlor ambient loop integrated (FreePD "Goodnightmare") and routed through AmbientSceneAudioController; downtown tension/combat stems now autogenerated via `scripts/audio/generateAr008AdaptiveStems.js` with loop metadata stored in `assets/generated/audio/ar-008/metadata.json` and `assets/music/requests.json`; outstanding work: run adaptive mix regression tests once layered playback hooks land.
