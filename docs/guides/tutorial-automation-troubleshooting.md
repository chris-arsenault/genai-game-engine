...
- **Save inspector metrics**: Opening the Save Inspector HUD overlay (default `[O]`, or `window.game.saveInspectorOverlay.show('automation')`) displays cascade event counts, hotspot rankings, and tutorial timelines for QA capture without devtools. Telemetry derives from `SaveManager.getInspectorSummary()` with store-based fallback, and `window.game.saveManager.exportInspectorSummary({ prefix: 'qa-run' })` will emit JSON/CSV artifacts for attaching to CI logs or manual QA notes.
