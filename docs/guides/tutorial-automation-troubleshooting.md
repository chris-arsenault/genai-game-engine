...
- **Transcript timeline (new)**: `SaveManager.getInspectorSummary().tutorial.transcript` now surfaces the bounded transcript recorded by `TutorialTranscriptRecorder`. While CSV/Markdown exports are pending, JSON artifacts (CLI + Playwright) already include the sequence array—assert key steps (e.g., `tutorial_step_started`, `tutorial_step_completed`) to confirm onboarding narratives progressed as expected.
