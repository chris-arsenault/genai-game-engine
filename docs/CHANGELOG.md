...
### Added
- Collision detectors now expose contact points and separation vectors with stricter validation coverage (`src/engine/physics/collisionDetectors.js`, `tests/engine/physics/collisionDetectors.test.js`).
- Investigation component introduced with ability sync, detective vision gating polish, and expanded system tests (`src/game/components/Investigation.js`, `src/game/systems/InvestigationSystem.js`, `tests/game/systems/InvestigationSystem.test.js`).
- Tutorial case scene metadata and witness configuration wired into the tutorial flow with data integrity tests (`src/game/data/cases/tutorialCase.js`, `src/game/managers/CaseManager.js`, `src/game/scenes/TutorialScene.js`, `tests/game/data/cases/tutorialCase.test.js`).
- Control bindings observation dwell metrics, blocked navigation ratios, and SaveInspector/exporter heuristics (`src/game/telemetry/ControlBindingsObservationLog.js`, `src/game/managers/SaveManager.js`, `src/game/ui/SaveInspectorOverlay.js`, `src/game/telemetry/inspectorTelemetryExporter.js`, `scripts/ux/exportControlBindingsObservations.js`, `tests/game/telemetry/ControlBindingsObservationLog.test.js`, `tests/game/managers/SaveManager.test.js`, `tests/game/ui/SaveInspectorOverlay.test.js`, `tests/game/telemetry/inspectorTelemetryExporter.test.js`, `tests/scripts/ux/exportControlBindingsObservations.test.js`).
