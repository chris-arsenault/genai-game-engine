...
- **CI artifact publisher**: `src/game/telemetry/CiArtifactPublisher.js` writes metadata manifests, executes optional upload commands with dependency-injected runners, and now orchestrates GitHub Actions REST/API fallbacks when primary commands are unavailable. Lifecycle events (`telemetry:ci_publish_started/completed/failed`) bubble through the EventBus for dashboards; Jest coverage (`tests/game/telemetry/CiArtifactPublisher.test.js`) asserts command invocation, metadata payloads, command-missing handling, and new fallback plumbing. Session #74 introduces `GitHubActionsArtifactFallback`, which uploads artifacts through `@actions/artifact` when the GitHub CLI cannot spawn, appends provider results directly into `ci-artifacts.json`, and keeps CI runs green while preserving telemetry attachments (unit coverage: `tests/game/telemetry/GitHubActionsArtifactFallback.test.js`).
