...
- **CI artifact publisher**: `src/game/telemetry/CiArtifactPublisher.js` writes metadata manifests, executes optional upload commands with dependency-injected runners, and now orchestrates GitHub Actions REST/API fallbacks when primary commands are unavailable. Metadata persists a `fallbackSummary` section (attempt counts, provider breakdowns, timestamps) so repeated fallbacks surface in the manifest. Lifecycle events (`telemetry:ci_publish_started/completed/failed`) bubble through the EventBus for dashboards; Jest coverage (`tests/game/telemetry/CiArtifactPublisher.test.js`) asserts command invocation, metadata payloads, fallback summary metrics, command-missing handling, and the GitHub Actions API integration (`tests/game/telemetry/GitHubActionsArtifactFallback.test.js`).
- **Performance baselines**: `npm run telemetry:performance:baseline` + `npm run telemetry:performance:summary` capture and evaluate five-run aggregates. The summary helper raises alerts when utilisation crosses 80%/95% of thresholds or peaks exceed caps; see `docs/performance/performance-baseline-guardrails.md` for retention and response playbooks.
...
- **Dispatch regression guardrail**
  - After integrating adapters or transcript recorders, rerun `node benchmarks/state-store-prototype.js` and compare `dispatchMeanMs` deltas (budget: +0.01 ms max variance, guardrail still ≤0.25 ms; latest mean ≈ 0.0108 ms with fallback wiring).
...
- **CI verification**
  ...
  - `.github/ci/telemetry-commands.json` wires in `node scripts/telemetry/providers/githubUpload.js`, which first attempts a native GitHub CLI upload, then transparently falls back to the Actions Artifact API (`@actions/artifact`) when the CLI is missing or fails. The provider captures stdout/stderr/exit codes, records fallback provenance, and persists provider results back to `ci-artifacts.json`.
  - Provider coverage lives in `tests/integration/githubUploadProvider.test.js`, which exercises dry-run skips, CLI-missing paths, Actions API fallbacks, failure handling for absent files, and happy-path uploads with captured metrics.
  ...
  - `scripts/telemetry/analyzeFallbackUsage.js` aggregates the new `fallbackSummary` metadata, exports markdown/JSON reports, and supports failure thresholds for repeated fallback attempts. Covered by `tests/scripts/telemetry/analyzeFallbackUsage.test.js` and exposed via `npm run telemetry:fallback-report`.
