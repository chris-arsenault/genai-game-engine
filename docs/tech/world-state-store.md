...
- **CI artifact publisher**: `src/game/telemetry/CiArtifactPublisher.js` writes metadata manifests and executes optional upload commands with dependency-injected runners. Lifecycle events (`telemetry:ci_publish_started/completed/failed`) bubble through the EventBus for dashboards; Jest coverage (`tests/game/telemetry/CiArtifactPublisher.test.js`) asserts command invocation, metadata payloads, and failure handling. Session #73 added a command-missing fallback that records a `status: "skipped"` result (exit `127`, `skippedReason: "command_not_found"`) instead of throwing, so CI runs tolerate temporary GitHub CLI outages while still surfacing telemetry artifacts.
